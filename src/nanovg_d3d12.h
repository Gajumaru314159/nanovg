...
#ifdef NANOVG_D3D12_IMPLEMENTATION
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>
...
struct D3DNVGtexture
{
	int id;
	//ID3D12Texture2D* tex;
	//ID3D12ShaderResourceView* resourceView;
	ID3D12Resource* tex;
	ID3D12DescriptorHeap* resourceView;
	int width, height;
	int type;
	int flags;
};
...
struct D3DNVGcontext
{
	...
	int ctextures;
	int textureId;
	//ID3D12SamplerState* pSamplerState[4];
	ID3D12Resource* pSamplerState[4];
	...
	//ID3D12BlendState* pBSNoWrite;

	//ID3D12RasterizerState* pRSNoCull;
	//ID3D12RasterizerState* pRSCull;

	//ID3D12DepthStencilState* pDepthStencilDrawShapes;
...
// BEGIN D3D specific
// TODO:シェーダ回り回収後D3Dnvg_copyMatrix3to4
...
static void D3Dnvg__setUniforms(struct D3DNVGcontext* D3D, int uniformOffset, int image)
{
	...
	D3D_API_2(D3D->pDeviceContext, Unmap, (ID3D12Resource*)D3D->pPSConstants, 0);
...
static void D3Dnvg__renderViewport(void* uptr, float width, float height, float devicePixelRatio)
{
	...
	D3D_API_5(D3D->pDeviceContext, Map, (ID3D12Resource*)D3D->pVSConstants, 0, D3D12_MAP_WRITE_DISCARD, 0, &mappedResource);
	memcpy(mappedResource.pData, &D3D->shader.vc, sizeof(struct VS_CONSTANTS));
	D3D_API_2(D3D->pDeviceContext, Unmap, (ID3D12Resource*)D3D->pVSConstants, 0);
}
...
static void D3Dnvg__fill(struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
	...
	D3D_API_1(D3D->pDeviceContext, IASetPrimitiveTopology, D3D12_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	for (i = 0; i < npaths; i++)
	{
	...
	D3D_API_1(D3D->pDeviceContext, IASetPrimitiveTopology, D3D12_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
...
static void D3Dnvg__convexFill(struct D3DNVGcontext* D3D, struct D3DNVGcall* call)
{
	...
	for (i = 0; i < npaths; i++)
	{
		// Draws a fan using indices to fake it up, since there isn't a fan primitive in D3D12.
